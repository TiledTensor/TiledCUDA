...
//                               const Element* xs, const Element* hs, Element*
//                               ts, const int m, const int n, const int k) {
...
//                                   const Element* o, const Element*
//                                   c_candidate, const Element* c, Element*
//                                   c_out, Element* h_out, int block_size, int
//                                   size) {
...
//         // reconsidering redistributing data to different threads and
//         performing
...
//             lstm_gate, cudaFuncAttributeMaxDynamicSharedMemorySize,
//             smem_size);
//     }
...
//     call, so the stacktrace below might be incorrect. For debugging consider
//     passing CUDA_LAUNCH_BLOCKING=1. Compile with `TORCH_USE_CUDA_DSA` to
...
//             reinterpret_cast<cutlass::half_t*>(h1.mutable_data_ptr()), m, n,
//             k);
//     } else {
