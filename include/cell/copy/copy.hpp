...
struct Shm2RegLoad {
   public:
    __device__ Shm2RegLoad(TiledCopy& copy, const STensor& src, DTensor& dst,
                           DTensorView& dst_view)
        : tiled_copy_(copy), src_(src), dst_(dst), dst_view_(dst_view) {}

    __device__ void copy(int pos) {
        cute::copy(tiled_copy_, src_(_, _, pos), dst_view_(_, _, pos));
    }

    __device__ int get_iters() { return size<2>(dst_); }

    __device__ const auto& operator[](int idx) { return dst_(_, _, idx); }
...
// __device__ auto& make_s2rA(const Element* data, int tid, const Layout&
// layout,
...
// template <typename Element, typename Layout, typename TiledMma>
// __device__ auto& make_s2rB(const Element* data, int tid, const Layout&
// layout,
//                            const TiledMma& tiled_mma);
