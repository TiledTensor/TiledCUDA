...
// FIXME(haruhi): a naive implementation to experiment with the interface.
// Consider better implementations.
template <typename Element, typename SrcLayout, typename DstLayout,
          typename ThreadLayout>
DEVICE void copy_2d_tile_s2r(const Element* src, int k,
                             const SrcLayout& s_layout, Element* dst,
                             const DstLayout& d_layout,
                             const ThreadLayout& t_layout) {
#ifdef DEBUG_
    {
        if (threadIdx.x == 0) {
    ...
    asm volatile(
        ...
        : "=r"(*reg_addr), "=r"(*(reg_addr + 1)), "=r"(*(reg_addr + 2)),
          "=r"(*(reg_addr + 3))
        : "r"(smem_addr));
