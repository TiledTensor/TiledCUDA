...
struct IndexedTensor_ {
    __device__ IndexedTensor_(Tensor& tensor) : tensor_(tensor) {}

    __device__ const auto& operator[](int idx) { return tensor_(_, _, idx); }
...
__device__ auto& convert_layout(const Tensor& acc) {
