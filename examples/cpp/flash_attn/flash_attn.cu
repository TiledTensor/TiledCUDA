...
int main() {
    ...
    thrust::host_vector<InType> hQ(N * D);
    for (int i = 0; i < hQ.size(); ++i) {
        hQ[i] = static_cast<InType>(rand_float());
    }

    thrust::host_vector<InType> hK(N * D);
    for (int i = 0; i < hK.size(); ++i) {
        hK[i] = static_cast<InType>(rand_float());
    }

    thrust::host_vector<InType> hV(N * D);
    for (int i = 0; i < hV.size(); ++i) {
        hV[i] = static_cast<InType>(rand_float());
    }

    thrust::host_vector<AccType> hO(N * D);
}
