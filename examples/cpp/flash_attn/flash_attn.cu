...
template <typename InType, typename OutType, typename IteratorQ, typename RegQ,
...
__global__ void flash_attn(const InType* dQ, const InType* dK, const InType* dV,
    ...
    for (int tr = 0; tr < IteratorQ::sc1; ++tr) {
        // Iterate over tr.
        // load Q tiles from global to register.
        // rQ: [B_r, d]
        loader_q(iter_g2r_q(tr), rQ);
