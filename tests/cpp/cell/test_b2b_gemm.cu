...
template <typename Element, typename ElementAcc,                     //
          ...
          typename GlobalC, typename SharedC, typename LoadSharedC,  //
          typename TileIteratorA, typename RegA, typename LoadRegA,
          typename TileIteratorB, typename RegB, typename LoadRegB,
          typename TileIteratorC, typename RegC, typename LoadRegC,
          typename RegAcc, typename GlobalD, typename RegD, typename StoreD>
__global__ void test_back2back_gemm(const Element* ga, const Element* gb,
                                    const Element* gc, ElementAcc* gd) {
    // A: m x k, B: k x n, C: n x p D: m x p
    GlobalA gA(ga);
    GlobalB gB(gb);
    // TODO(KuangjuX): Compute offset for global C.
    GlobalC gC(gc);

    extern __shared__ __align__(sizeof(double)) unsigned char buf_[];
    auto* shared_a = reinterpret_cast<Element*>(buf_);
    auto* shared_b = shared_a + TileIteratorA::Tile::kNumel;
    auto* shared_c = shared_b + TileIteratorB::Tile::kNumel;
...
void run_test() {
    ...
    //         __half*>(thrust::raw_pointer_cast(d_b.data())),
    //         reinterpret_cast<const
    //         __half*>(thrust::raw_pointer_cast(d_a.data())),
